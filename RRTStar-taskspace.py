...
class RRTStar:
    ...
    def __init__(self, robot, gamma_rrt_star=1.0, eta=0.1, max_iterations=10000, goal_threshold=0.08, goal_bias=0.5,with_visualization=False):
        self.robot = robot
        self.tree = []
    ...
    # check : we dont need start pos because it is starting from where the robot currently is
    ...
    def steer(self, start_pos, target_pos):
        ...
        if distance <= self.eta:
            return self.robot.inverse_kinematics(target_pos,[np.pi/2, 0, 0]) # [np.pi, 0, 0]
        else:
            direction = (direction / distance) * self.eta
            new_target_pos = start_pos + direction
            return self.robot.inverse_kinematics(new_target_pos,[np.pi/2, 0, 0]) # [np.pi, 0, 0]
